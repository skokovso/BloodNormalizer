...
Версия 2.1 - с папкой "данные" и исправлением ошибок

Что делает:
1. Ищет исходный файл в папке "данные"
...
def main():
    print("🩸 ЗАПУСК ПРЕПРОЦЕССОРА ТРАНСФУЗИЙ v2.1")
    print("="*60)
    ...
    # Формируем путь к файлу в папке "данные"
    input_path = Path(CONFIG['input_folder']) / CONFIG['input_file']
    
    if not input_path.exists():
        print(f"❌ Файл не найден: {input_path}")
        print(f"   Убедитесь, что файл '{CONFIG['input_file']}' находится в папке '{CONFIG['input_folder']}'")
        print(f"   Полный путь: {input_path.absolute()}")
        return
    
    print(f"📂 Загружаем файл: {input_path}")
    
    # Пробуем загрузить файл с разными engine
    try:
    ...
    problems = data[
        ...
        (data['Blood_Group_Patient_Full'].astype(str).str.contains('\?', na=False))
    ]
