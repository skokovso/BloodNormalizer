...
Пример использования:
    python preprocess_blood_data_v2_4.py
    
Входные данные:
    Excel-файл с трансфузиями в папке 'данные/'
    
Выходные данные:
    - Файл с префиксом '_preprocessed_v2' (нормализованные данные)
    - Файл с отчетом о проблемных записях (при наличии)
...
CONFIG = {
    ...
    'output_suffix': '_preprocessed_v2',   # Суффикс для выходного файла
    ...
    'component_keywords': {
        ...
        'Плазма': ['плазм', 'plasma', 'свежезамороженная плазма'],
        'Тромбоциты': ['тромбоцит', 'platelet', 'тромбоконцентрат', 'тромбоцитарный'],
...
def select_file_from_data_folder() -> Optional[Path]:
    ...
    # Проверяем/создаем папку с данными
...
def find_header_row(df: pd.DataFrame, required_cols: List[str]) -> Tuple[Optional[int], Optional[Dict]]:
    ...
    for idx in CONFIG['search_rows']:
        ...
        # Безопасное преобразование всех значений в строки
        row_values = []
...
def main() -> None:
    ...
    """
    print("🩸 ЗАПУСК ПРЕПРОЦЕССОРА ТРАНСФУЗИЙ v2.4")
    print("="*60)
