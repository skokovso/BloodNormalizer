...
Изменения в v8.0:
    ...
    - Добавлены столбцы "ФИО_пациента" и "Номер_пациента" в лист проблем
"""
...
def main() -> None:
    ...
    time_to_col = None
    phenotype_col = None
    patient_fio_col = None      # ФИО пациента
    patient_id_col = None       # № пациента
    ...
    for col in data.columns:
        ...
        if ('ab0' in col_lower or 'abo' in col_lower):
            ...
            phenotype_col = col
        elif 'фио' in col_lower and ('пациент' in col_lower or 'пац' in col_lower):
            patient_fio_col = col
        elif 'номер' in col_lower and ('пациент' in col_lower or 'пац' in col_lower):
            patient_id_col = col
        elif col_lower == '№ пациента':
            patient_id_col = col
    ...
    if CONFIG['debug']:
        ...
        print(f"   ФИО пациента: {patient_fio_col}")
        print(f"   № пациента: {patient_id_col}")
    ...
    # Добавляем ФИО и № пациента, если они есть
    if patient_fio_col:
        data['ФИО_пациента'] = data[patient_fio_col]
    if patient_id_col:
    ...
    try:
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            # Лист с проблемами нормализации (с ФИО и № пациента)
            problems_all = data[(data['Проблема_пациента'] != '') | (data['Проблема_среды'] != '')]
            if len(problems_all) > 0:
                # Базовые колонки с проблемами
                problem_cols = ['Проблема_пациента', 'Проблема_среды', 
                               'Blood_Group_Patient_Full', 'Blood_Group_Env_Full',
                               'Component_Type']
                
                # Добавляем ФИО и № пациента, если они есть в данных
                if 'ФИО_пациента' in data.columns:
                    problem_cols.insert(0, 'ФИО_пациента')
                if 'Номер_пациента' in data.columns:
                    problem_cols.insert(1, 'Номер_пациента')
