...
"""
Расширенный анализ иногруппных трансфузий v4.2
===============================================
...
Ищет самый свежий нормализованный файл, так как теперь меняются суффиксы нормализованных файлов в соответсвии с именем файла нормализатора
...
# =============================================================
# АВТОМАТИЧЕСКИЙ ПОИСК НОРМАЛИЗОВАННОГО ФАЙЛА (самый свежий)
# =============================================================

def find_normalized_file(data_folder: str = 'enter_data') -> Path:
    """
    Автоматически ищет нормализованный файл в папке data_folder.
    Возвращает самый свежий (последний по дате изменения).
    
    Приоритет:
    1. Файл с суффиксом _by_blood_normalizer
    2. Файл с суффиксом _normalize
    3. Любой файл, содержащий 'normalized' или 'preprocessed'
    4. Самый свежий xlsx файл в папке
    """
    ...
    if not all_files:
        raise FileNotFoundError(f"В папке '{data_folder}' нет Excel файлов!")
    
    # Группируем файлы по категориям
    blood_normalizer_files = [f for f in all_files if '_by_blood_normalizer' in f.stem]
    normalize_files = [f for f in all_files if f.stem.endswith('_normalize')]
    normalized_files = [f for f in all_files if 'normalized' in f.stem or 'preprocessed' in f.stem]
    
    # Сортируем каждую группу по дате изменения (свежие первыми)
    blood_normalizer_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
    normalize_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
    normalized_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
    all_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
    
    # Приоритет 1: файл с суффиксом _by_blood_normalizer (самый свежий)
    if blood_normalizer_files:
        latest = blood_normalizer_files[0]
        print(f"   ✅ Найден файл от нормализатора (свежий): {latest.name}")
        print(f"      (дата: {datetime.fromtimestamp(latest.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')})")
        return latest
    
    # Приоритет 2: файл с суффиксом _normalize (самый свежий)
    if normalize_files:
        latest = normalize_files[0]
        print(f"   ✅ Найден нормализованный файл (свежий): {latest.name}")
        print(f"      (дата: {datetime.fromtimestamp(latest.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')})")
        return latest
    
    # Приоритет 3: файл содержащий 'normalized' или 'preprocessed' (самый свежий)
    if normalized_files:
        latest = normalized_files[0]
        print(f"   ✅ Найден обработанный файл (свежий): {latest.name}")
        print(f"      (дата: {datetime.fromtimestamp(latest.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')})")
        return latest
    
    # Приоритет 4: самый свежий xlsx файл (с предупреждением)
    latest = all_files[0]
    print(f"   ⚠️ Нормализованный файл не найден. Использую самый свежий: {latest.name}")
    print(f"      (дата: {datetime.fromtimestamp(latest.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')})")
    print("   (рекомендуется предварительно запустить blood_normalizer_v_8_stable.py)")
    return latest


# Определяем входной файл автоматически (самый свежий)
try:
    INPUT_FILE = find_normalized_file('enter_data')
...
COMPATIBILITY_ERYTHROCYTE = {
    ...
    'B+': ['B+', 'B-', 'O+', 'O-'],
    'B-': ['B-', 'O-'],
    'AB+': ['AB+', 'AB-', 'A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'A2+', 'A2-', 'A2B+', 'A2B-'],
    'AB-': ['AB-', 'A-', 'B-', 'O-', 'A2-', 'A2B-'],
...
def main():
    ...
    with pd.ExcelWriter(report_path, engine='openpyxl') as writer:
        ...
        if len(incompatible_df) > 0:
            cols = ['Год', date_col, DOCTOR_COLUMN, 'Структура', 'ФИО пациента', '№ пациента', 'Component_Type',
